...
def dense_hoscpool(
    ...
    # if anchor_adj:
    anchor_adj = anchor_adj.unsqueeze(0) if anchor_adj.dim() == 2 else anchor_adj
    motif_adj = torch.matmul(anchor_adj, anchor_adj.transpose(1, 2))
    motif_out_adj = torch.matmul(torch.matmul(s.transpose(1, 2), motif_adj), s)
    ...
    return out, out_adj, mu * hosc_loss, mu * ortho_loss
    # return out, out_adj, hosc_loss, ortho_loss
