...
def aug_random_edge(A, drop_percent=0.2):
    ...
    """
    input_adj = sp.csr_matrix(A.numpy()) # A.numpy(force=True)
    ...
    new_adj = sp.csr_matrix((data, (row_idx, col_idx)), shape=input_adj.shape).toarray()

    return torch.tensor(new_adj).float()
...
def phi(feature, true_labels, cluster_num):
    ...
    # predict_labels, centers = kmeans(X=feature, num_clusters=cluster_num, distance="cosine", device="cuda")
...
def comprehensive_similarity(H1, H2):
